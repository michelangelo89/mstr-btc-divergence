...
\[
\log(MSTR_t) = \alpha + \beta_t \cdot \log(BTC_t) + \epsilon_t
\]

- `β_t`: dynamic hedge ratio (estimated via **rolling** or **exponentially weighted (EWMA)** regression on log-returns).  
- `ε_t`: residual spread representing short-term divergence.
